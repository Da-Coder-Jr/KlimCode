	...
	<div class="max-w-2xl mx-auto px-4 py-8">
		<div class="mb-8">
			<div class="flex items-center gap-3 mb-1">
				<svg class="w-6 h-6" style="color: var(--content-secondary)" fill="currentColor" viewBox="0 0 24 24">
		...
		<div class="space-y-6">
			...
			{#if $githubConnected}
				<section class="card p-5">
					<h2 class="text-[15px] font-semibold mb-4" style="color: var(--content)">Repository</h2>
					<RepoSelector />
				</section>
				...
				{#if $selectedRepo}
					<section class="card p-5">
						<div class="flex items-center justify-between mb-4">
							<h2 class="text-[15px] font-semibold" style="color: var(--content)">Pull Requests</h2>
							<span class="text-xs font-mono" style="color: var(--content-muted)">{$selectedRepo.fullName}</span>
						</div>
						<PRList />
					</section>
				{/if}
				...
				<section class="card p-5">
					<h2 class="text-[15px] font-semibold mb-4" style="color: var(--content)">How Agent Creates PRs</h2>
					<div class="space-y-3">
						{#each [
							...
							<div class="flex gap-3 items-start">
								<div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style="background-color: var(--accent-subtle); border: 1px solid var(--border)">
									<span class="font-bold text-xs" style="color: var(--content-tertiary)">{item.step}</span>
								</div>
								<div>
									<div class="text-sm font-medium" style="color: var(--content-secondary)">{item.title}</div>
									<div class="text-xs" style="color: var(--content-muted)">{item.desc}</div>
								</div>
							</div>
						{/each}
					</div>
