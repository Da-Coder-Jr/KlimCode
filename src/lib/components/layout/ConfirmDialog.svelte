...
{#if open}
	...
	>
		<div class="absolute inset-0 bg-black/40 dark:bg-black/70 backdrop-blur-sm"></div>

		<div
			class="relative rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden"
			style="background-color: var(--surface); border: 1px solid var(--border)"
			transition:fly={{ y: 40, duration: 250, easing: cubicOut }}
		>
			<div class="p-5">
				<!-- Icon -->
				<div class="w-11 h-11 rounded-xl {variantStyles.iconBg} flex items-center justify-center mx-auto mb-4">
				...
				<!-- Title -->
				<h3 class="text-[15px] font-semibold text-center mb-1.5" style="color: var(--content)">{title}</h3>

				<!-- Message -->
				{#if message}
					<p class="text-sm text-center leading-relaxed" style="color: var(--content-tertiary)">{message}</p>
				{/if}
			</div>
