...
<aside class="flex flex-col h-full w-full" style="background-color: var(--sidebar-bg); backdrop-filter: blur(12px); border-right: 0.5px solid var(--sidebar-border)">
	<!-- Header -->
	<div class="flex items-center justify-between px-3 pt-3 pb-1.5">
		<a href="/" class="flex items-center gap-2.5 group hover:opacity-80 transition-opacity">
			...
			<span class="font-semibold text-sm tracking-tight" style="color: var(--content)">KlimCode</span>
		</a>
		<button
			on:click={() => showNewMenu = !showNewMenu}
			class="p-1.5 rounded-lg transition-all hover:bg-black/5 dark:hover:bg-white/5"
			title="New chat"
		>
			<svg class="w-4.5 h-4.5" style="color: var(--content-tertiary)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	<!-- New chat dropdown -->
	{#if showNewMenu}
		<div class="mx-3 mb-2 rounded-xl overflow-hidden z-50 shadow-lg animate-slide-down" style="background-color: var(--surface); border: 1px solid var(--border)">
			<button
				on:click={handleNewChat}
				class="w-full px-3.5 py-2.5 text-left text-sm flex items-center gap-3 transition-colors hover:bg-black/5 dark:hover:bg-white/5"
				style="color: var(--content)"
			>
				<div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style="background-color: var(--surface-tertiary)">
				...
				<div>
					...
					<div class="text-[11px]" style="color: var(--content-muted)">Ask questions, get help</div>
				</div>
			</button>
			...
			>
				...
				<div>
					...
					<div class="text-[11px]" style="color: var(--content-muted)">Write code, create PRs</div>
				</div>
			</button>
		</div>
	{/if}
		...
		<div class="px-3 pb-2">
			<div class="relative">
				...
				<input
					...
					style="background-color: transparent; border: 1px solid transparent; color: var(--content-secondary)"
				/>
				{#if searchQuery}
					<button
		...
		{#each [
			...
			{#if group.items.length > 0}
				<div class="px-2 pt-3.5 pb-1">
					<span class="text-[10px] font-medium uppercase tracking-wider" style="color: var(--content-muted)">{group.label}</span>
				</div>
				{#each group.items as conv (conv.id)}
					<a
						href="/chat/{conv.id}"
						on:click|preventDefault={() => handleSelect(conv.id)}
						class="group/item flex h-[2.25rem] items-center gap-2 rounded-2xl pl-2.5 pr-2 transition-all duration-150"
					...
					>
						<div class="flex-shrink-0">
							...
							{:else}
								<div class="w-1.5 h-1.5 rounded-full" style="background-color: var(--content-muted); opacity: 0.5"></div>
							{/if}
						</div>
						...
						<div class="hidden group-hover/item:flex items-center gap-0.5 flex-shrink-0">
							<button
								on:click|preventDefault|stopPropagation={(e) => startRename(e, conv)}
								class="p-1 rounded-lg transition-colors hover:bg-black/5 dark:hover:bg-white/5"
								style="color: var(--content-muted)"
								title="Rename"
							>
								<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							...
							<button
								...
								class="p-1 rounded-lg text-red-500 hover:text-red-600 dark:hover:text-red-400 transition-colors hover:bg-red-500/5"
								title="Delete"
							>
								<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	<!-- Bottom Nav -->
	<div class="p-2 space-y-0.5" style="border-top: 0.5px solid var(--sidebar-border)">
		<button
			on:click={cycleTheme}
			class="w-full flex items-center gap-3 px-2.5 py-2 rounded-2xl text-sm transition-all duration-150 hover:bg-black/5 dark:hover:bg-white/5"
		...
		>
			...
			{/if}
			<span class="text-[13px]">{$themePreference === 'system' ? 'System' : $themePreference === 'dark' ? 'Dark' : 'Light'}</span>
		</button>

		<a
			href="/github"
			class="flex items-center gap-3 px-2.5 py-2 rounded-2xl text-sm transition-all duration-150 hover:bg-black/5 dark:hover:bg-white/5"
	...
	<!-- User -->
	{#if $currentUser}
		<div class="px-2 pb-2 relative" style="border-top: 0.5px solid var(--sidebar-border)">
			<button
				on:click={() => showUserMenu = !showUserMenu}
				class="w-full flex items-center gap-3 px-2.5 py-2.5 rounded-2xl transition-all hover:bg-black/5 dark:hover:bg-white/5"
			>
			...
			{#if showUserMenu}
				<div class="absolute bottom-full left-2 right-2 mb-1 rounded-xl shadow-lg overflow-hidden z-50 animate-slide-up" style="background-color: var(--surface); border: 1px solid var(--border)">
					<button
						on:click={handleLogout}
						class="w-full px-4 py-2.5 text-left text-sm flex items-center gap-2.5 transition-colors hover:bg-black/5 dark:hover:bg-white/5"
						style="color: var(--content-tertiary)"
					>
