<script lang="ts">
	import { githubConnected, githubUsername, getConnectUrl, disconnectGitHub, checkGitHubConnection } from '$stores/github';
	...
	<div class="flex items-center gap-3 p-3.5 rounded-xl" style="background-color: var(--surface-tertiary); border: 1px solid var(--border)">
		...
		<div class="flex items-center gap-2">
			<button
				...
				style="color: var(--content-tertiary); border-color: var(--border)"
			...
			<button
				...
				class="text-xs font-medium px-3 py-1.5 rounded-lg hover:bg-red-500/10 text-red-500 dark:text-red-400 transition-all border border-transparent hover:border-red-500/20"
			>
	...
	<div class="{compact ? '' : 'text-center'}">
		{#if !compact}
			<div class="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style="background-color: var(--surface-tertiary); border: 1px solid var(--border)">
				<svg class="w-7 h-7" style="color: var(--content-tertiary)" fill="currentColor" viewBox="0 0 24 24">
			...
			<p class="text-sm mb-5 max-w-xs mx-auto leading-relaxed" style="color: var(--content-muted)">
		...
		<button
			...
			disabled={isConnecting}
			class="btn-github {compact ? 'text-sm' : ''} hover:shadow-soft active:scale-[0.98] transition-all disabled:opacity-50"
		>
			{#if isConnecting}
