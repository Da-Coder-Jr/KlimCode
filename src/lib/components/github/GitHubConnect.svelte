...
{#if $githubConnected}
	<div class="flex items-center gap-3 p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/15">
		...
		<div class="flex-1 min-w-0">
			<div class="text-sm text-zinc-200 font-medium">Connected to GitHub</div>
			<div class="text-xs text-zinc-500">@{$githubUsername}</div>
		</div>
		<button on:click={handleDisconnect} class="text-xs text-zinc-500 hover:text-red-400 px-2 py-1 rounded-lg hover:bg-red-500/10 transition-all">
			Disconnect
		</button>
	</div>
{:else}
	<div class="{compact ? '' : 'text-center'}">
		{#if !compact}
			<div class="w-12 h-12 rounded-xl bg-zinc-800 flex items-center justify-center mx-auto mb-3 border border-zinc-700">
